...
public class KMeansPlusPlus {
    ...
    // Elbow point를 찾기 위해 기울기 변화를 계산합니다.
        int bestK = 1;
        double maxElbowValue = Double.NEGATIVE_INFINITY;

        for (int k = 1; k < maxK - 1; k++) {
            double currentSlope = costs[k] - costs[k - 1];
            double nextSlope = costs[k + 1] - costs[k];
            double elbowValue = currentSlope - nextSlope;

            if (elbowValue > maxElbowValue) {
                maxElbowValue = elbowValue;
                bestK = k + 1;
            }
        }
